...
export default function Insights() {
  ...
  const handleCardClick = (instanceId: string) => {
      navigate(`/app/instance/${instanceId}/chat`);
  };
    ...
    <div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {bookmarks.map((item, index) => (
          <motion.div
            ...
            className="group bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm hover:shadow-md hover:border-zinc-300 transition-all duration-200 flex flex-col cursor-pointer"
          >
            <div className="flex items-start justify-between mb-4">
                ...
                <button 
                    ...
                    className="text-purple-400 hover:text-zinc-300 transition-colors"
            ...
            <h3 className="text-sm font-semibold text-zinc-900 mb-2 line-clamp-2">
                "{item.query}"
            </h3>
            
            <div className="flex-1 bg-zinc-50 border border-zinc-100 rounded-xl p-3 mb-4 group-hover:bg-zinc-100/50 transition-colors">
            ...
            <div className="pt-4 border-t border-zinc-100 flex items-center justify-between mt-auto">
                ...
                <div className="text-xs font-medium text-purple-600 group-hover:translate-x-1 transition-transform flex items-center gap-1">
                    Open Context
      ...
      {bookmarks.length === 0 && (
          <div className="text-center py-20 bg-zinc-50 rounded-2xl border border-zinc-200 border-dashed">
              <div className="w-12 h-12 bg-zinc-200 rounded-full flex items-center justify-center mx-auto mb-4 text-zinc-400">
                  <Bookmark className="w-6 h-6" />
              </div>
              <p className="text-zinc-600 font-medium">No saved insights</p>
              <p className="text-zinc-400 text-sm">Bookmark AI responses in the chat interface to save them here.</p>
          </div>
      )}
    </div>
  );
